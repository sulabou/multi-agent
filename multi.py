...
def handle_trace_event(event):
    ...
    agent_display_names = {
        ...
        "security-expert": "セキュリティ診断エージェント",
...
def main():
    ...
    if st.session_state.clients and "cloudwatch" in st.session_state.clients:
        ...
        with col1:
            ...
            st.caption(f"最終更新: {japan_time.strftime('%Y-%m-%d %H:%M:%S')} (JST)")
    ...
    if st.session_state.alarms:
        selected_alarm, selected_index = display_alarm_selection(st.session_state.alarms)  # 戻り値を2つに変更
